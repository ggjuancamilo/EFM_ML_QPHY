...
K =20 # number of branches
Nh0 = 3 # number of nodes in the hidden Layer
Nh1 = 5
...
#One Hidden layer procedure

### Layer hidden: ###
W0 = tf.Variable( tf.random_normal([2, Nh0], mean=0.0, stddev=0.01, dtype=tf.float32) )
b0 = tf.Variable( tf.zeros([Nh0]) )
z0 = tf.matmul(x, W0) + b0
a0 = tf.nn.sigmoid( z0 )

### Layer hidden: ###
W2 = tf.Variable( tf.random_normal([2, Nh2], mean=0.0, stddev=0.01, dtype=tf.float32) )
b2 = tf.Variable( tf.zeros([Nh2]) )
z2 = tf.matmul(a0, W2) + b2
a2 = tf.nn.sigmoid( z2 )


### Final Layer:###

W1 = tf.Variable( tf.random_normal([Nh2, K], mean=0.0, stddev=0.01, dtype=tf.float32) )
b1 = tf.Variable( tf.zeros([K]) )
z1 = tf.matmul(a2, W1) + b1
a1 = tf.nn.sigmoid( z1 )
...
# ##Standart procedure
# ### Layer hidden: ###
# W0 = tf.Variable( tf.random_normal([2, K], mean=0.0, stddev=0.01, dtype=tf.float32) )
